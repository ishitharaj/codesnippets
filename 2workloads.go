...
func (in *WorkloadService) fetchWorkloadsFromCluster(ctx context.Context, cluster string, namespace string, labelSelector string) (models.Workloads, error) {
	...
	resourceTypes := []string{
		"pods", "replicationcontrollers", "deployments", "replicasets",
		"deploymentconfigs", "statefulsets", "jobs", "cronjobs", "daemonsets",
	}

	for _, resourceType := range resourceTypes {
		allowed, err := CheckUserPermissions(ctx, userClient, in.businessLayer.Permissions.ResourcePermissions[resourceType], resourceType, "list")
		if err != nil {
			log.Errorf("Error checking permissions for resource %s: %v", resourceType, err)
			continue
		}
		if !allowed {
			log.Infof("User does not have permission to list %s in namespace %s", resourceType, namespace)
			continue
		}

		// Fetch the resource if user has permission
		switch resourceType {
		case "pods":
			pods, err = kubeCache.GetPods(namespace, labelSelector)
		case "replicationcontrollers":
			repcon, err = kubeCache.GetReplicationControllers(namespace)
		case "deployments":
			dep, err = kubeCache.GetDeployments(namespace)
		case "replicasets":
			repset, err = kubeCache.GetReplicaSets(namespace)
		case "deploymentconfigs":
			depcon, err = kubeCache.GetDeploymentConfigs(namespace)
		case "statefulsets":
			fulset, err = kubeCache.GetStatefulSets(namespace)
		case "jobs":
			jbs, err = kubeCache.GetJobs(namespace)
		case "cronjobs":
			conjbs, err = kubeCache.GetCronJobs(namespace)
		case "daemonsets":
			daeset, err = kubeCache.GetDaemonSets(namespace)
		}

		if err != nil {
			log.Errorf("Error fetching %s per namespace %s: %s", resourceType, namespace, err)
			continue
		}
	}

	// Convert to Kiali model
	ws = in.buildWorkloadList(cluster, namespace, pods, repcon, dep, repset, depcon, fulset, jbs, conjbs, daeset)
...
func (in *WorkloadService) buildWorkloadList(cluster string, namespace string, pods []core_v1.Pod, repcon []core_v1.ReplicationController, dep []apps_v1.Deployment, repset []apps_v1.ReplicaSet, depcon []osapps_v1.DeploymentConfig, fulset []apps_v1.StatefulSet, jbs []batch_v1.Job, conjbs []batch_v1.CronJob, daeset []apps_v1.DaemonSet) models.Workloads {
	ws := models.Workloads{}
	controllers := map[string]string{}
	...
	for _, controllerName := range controllerNames {
		...
		case kubernetes.ReplicationControllerType:
			for _, rc := range repcon {
				if rc.Name == controllerName {
					...
					w.SetPods(kubernetes.FilterPodsBySelector(selector, pods))
					w.ParseReplicationController(&rc)
					break
				}
			}
		case kubernetes.DeploymentConfigType:
		...
		case kubernetes.StatefulSetType:
			for _, fs := range fulset {
				if fs.Name == controllerName {
					selector := labels.Set(fs.Spec.Template.Labels).AsSelector()
					w.SetPods(kubernetes.FilterPodsBySelector(selector, pods))
					w.ParseStatefulSet(&fs)
					break
				}
			}
		case kubernetes.PodType:
		...
		case kubernetes.CronJobType:
			for _, cjb := range conjbs {
				if cjb.Name == controllerName {
					selector := labels.Set(cjb.Spec.JobTemplate.Spec.Template.Labels).AsSelector()
					w.SetPods(kubernetes.FilterPodsBySelector(selector, pods))
					w.ParseCronJob(&cjb)
					break
				}
